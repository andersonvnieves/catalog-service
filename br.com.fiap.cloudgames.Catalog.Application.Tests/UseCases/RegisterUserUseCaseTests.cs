...
namespace br.com.fiap.cloudgames.Catalog.Application.Tests.UseCases;
...
public class RegisterUserUseCaseTests
{
    ...
    public async Task ExecuteAsync_WithValidRequest_ShouldCreateIdentityUser_ThenPersistDomainUser_AndCommit()
    {
        ...
        repo.Setup(x => x.AddAsync(It.IsAny<br.com.fiap.cloudgames.Catalog.Domain.Aggregates.User>()))
            .Returns(Task.CompletedTask);
        uow.Setup(x => x.CommitAsync()).Returns(Task.CompletedTask);
        publisher.Setup(x => x.PublishAsync(It.IsAny<br.com.fiap.cloudgames.Catalog.Application.Events.UserCreatedEvent>()))
            .Returns(Task.CompletedTask);
        ...
        repo.Verify(x => x.AddAsync(It.IsAny<br.com.fiap.cloudgames.Catalog.Domain.Aggregates.User>()), Times.Once);
        uow.Verify(x => x.CommitAsync(), Times.Once);
        uow.Verify(x => x.RollbackAsync(), Times.Never);
        publisher.Verify(x => x.PublishAsync(It.IsAny<br.com.fiap.cloudgames.Catalog.Application.Events.UserCreatedEvent>()), Times.Once);
    }
    ...
    public async Task ExecuteAsync_WhenAuthServiceThrows_ShouldRollback_AndRethrow()
    {
        ...
        repo.Verify(x => x.AddAsync(It.IsAny<br.com.fiap.cloudgames.Catalog.Domain.Aggregates.User>()), Times.Never);
